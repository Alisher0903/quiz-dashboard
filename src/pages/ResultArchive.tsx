...
const ResultArchive = () => {
  ...
  const groupedResults = resultList ? resultList.reduce((acc: any, curr) => {
    ...
    <>
      ...
      <div className="space-y-4">
        {isLoading ? <PendingLoader /> : (
          resultList && Object.keys(groupedResults).length > 0 ? Object.keys(groupedResults).map((category, catIdx) => (
            <div key={catIdx}>
              {/* Display category name once */}
              <div className="py-5 mt-6 md:mt-9 lg:mt-16">
                <p className={`lg:text-4xl md:text-2xl text-xl text-center font-semibold`}>{category}</p>
                <p className={'flex items-center justify-center gap-5 text-base flex-wrap'}>
                  ...
                  <span>Қийин саволлар сони: *** та</span>
                  <span>Ўрта саволлар сони: *** та</span>
                  <span>Осон саволлар сони: *** та</span>
                </p>
              </div>
              ...
              {groupedResults[category].map((q: any, index: number) => (
                ...
                >
                  ...
                  <p className={`dark:text-form-input`}>
                    <span
                      className="font-semibold">{!q.correctAnswer ? 'Жавобингиз туғри' : 'Сизнинг жавобингиз'}: </span>
